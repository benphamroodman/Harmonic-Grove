    ...
    void Update()
    {
        ...
        if (targetObject != null)
        {
            ...
            if (direction.magnitude > 0)
            {
                ...
                // If the object contains "triangle", rotate it 90 degrees on the x-axis as well (so it looks sideways)
                if (transform.name.ToLower().Contains("triangle"))
                {
                    // Rotate the object to face the target's position
                    // transform.rotation = Quaternion.LookRotation(direction);
                    transform.Rotate(0, 1, 0, Space.World);  // Rotate 90 degrees on the x-axis
